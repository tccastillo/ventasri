...
//**-------- Instanciando Select Cliente
    ...
    $('#cbCliente').change(function() {
      ...
      $.getJSON('sri/contarcredito.php?cliente='+cliente,function(data){
           $("#txtContarCredito").val(data);
           if($("#chkPagado").prop('checked') == false){
               Venta_Credito();
             }
       });
    ...
    $(document).keydown(function(objEvent) {
        if (objEvent.keyCode == 115) {  //clicked F4
            if ($('#tbldetalle > tbody >tr').length > 0){
              $('#modal_iconified_cash').modal('show'); //do click
          //  alert('ESTOY APRETANDO F2');
          }
        }
    });
buscar_por_codigo();
...
function Venta_Credito(){
  ...
  var contar_credito = $("#txtContarCredito").val();

  if(parseFloat(monto_deudor) > parseFloat(limite_credito) || parseFloat(limite_credito) == 0.00 || limite_credito == '' ){
    $("#btnRegistrar").prop("disabled",true);
    
  } else if(contar_credito<1) {
    $("#btnRegistrar").prop("disabled",true);
    alert("AUN TIENE FACTURAS PENDIENTES DE PAGO");
  } else {
    $("#btnRegistrar").prop("disabled",false);
  }
    ...
    var filas=0;
      ...
      if(inventariable == 0){
                ...
                '<span class="text-muted">'+especificacion+'</span></td>';
                 ...
                 tr_add += '<td width="5%"><input type="text" id="tblprecio" name="tblprecio" value="'+precio_venta+'" class="touchspin" style="width:70px;"></td>';
                 ...
                 if(existe==false){
                   ...
                   $("input[name='tblprecio']").TouchSpin({
                       verticalbuttons: true,
                       verticalupclass: 'icon-arrow-up22',
                       verticaldownclass: 'icon-arrow-down22',
                       min: 1.00,
          ...
          if(perecedero == 0){
                ...
                '<span class="text-muted">'+especificacion+'</span></td>';
                 ...
                 if(idproducto=='15'){
                  tr_add += '<td align="center">'+precio_venta+' <input type="hidden" id="tblprecio"  value="'+precio_venta+'"  style="width:70px;"> </td>';
                 }else{
                  tr_add += '<td width="5%"><input type="text" id="tblprecio" name="tblprecio" value="'+precio_venta+'" class="touchspin" style="width:70px;"></td>';
                 }
...
$(document).on("focusout","#tblcant, #tbldesc, #tblprecio",function(){
    totales();
})
...
var total_sumas=0; total_exentos=0; total_iva = 0; subtotal=0; totalessuma=0; sumas = 0; iva = 0; subtotal = 0;exentos = 0; total=0;descuentos = 0;
         ...
         $.each(data,function(key,val){
           ...
           iva_div = iva + 1;
            ...
            $("#tbldetalle tbody tr").each(function (index)
                {
                    var campo1, campo2, campo3, campo4, campo5, campo6, campo7, campo8, campo9, campo10, campo11, sumaiva;
                    $(this).children("td").each(function (index2)
                    {
                        switch (index2)
                        {
                            ...
                            case 3:  campo3 = $(this).find("#tblprecio").val();
                            ...
                            case 6: campo6=campo2 * campo3 ;
                                    if(campo5>0){
                                      ...
                                      if(campo4!=0.00)
                                       {
                                        sumastotal = (campo2 * campo3);
                                        ivaproducto=0.00;
                                        valortotal=(campo2 * campo3)-campo5;
                                       } else {
                                         sumastotal = ((campo2 * campo3)/1.12)-campo5;
                                        ivaproducto=sumastotal*0.12;
                                        valortotal=sumastotal+ivaproducto;
                                       }
                                      ...
                                      if(campo4!=0.00)
                                       {
                                        sumastotal = (campo2 * campo3);
                                         ...
                                         sumastotal = (campo2 * campo3)/1.12;
                                       }
                                      
                                      ivaproducto=campo6-sumastotal;
                                      valortotal=sumastotal+ivaproducto;
                                    }
                                    totalesnew = parseFloat(valortotal);
                                    sumas = parseFloat(sumastotal);
                                    if(isNaN(sumas)){sumas = 0.00;}
                                    $(this).text(sumas.toFixed(2));
                                    break;

                            case 7:  campo7 = (campo2 * campo3) - campo5;
                                     break;
                      ...
                      totalessuma=(totalessuma + totalesnew)-exentos;
                      total_sumas = subtotal;
                      total_iva = ivaproducto+total_iva;
                      ...
                      if(total_sumas >= monto_retencion){
                         ...
                         total = (totalessuma + total_exentos) ;
        ...
        if(total > 0.00){
            ...
            success: function(data){
              ...
              if(data=="Validado"){
                ...
                VentanaCentrada('./sri/index.php?id_venta='+ comprobante + '&id_cliente=' +idcliente,'Enviando a SRI','','650','600','true');
