...
$contenido=file_get_contents('firmados/'.$_GET['archivo']);
...
try {
	...
	$parametrosautoriza= array("claveAccesoComprobante" => $_GET['clave']);
	$imprime=array();
	...
	try {
		...
		if ($respuesta==='AUTORIZADO') {
			...
			$xml->appendChild($etqautoriza);
				...
				$nivel = VentaData::getById($_GET['id_venta']);
				$nivel->aceptado = "AUTORIZADO";
				$nivel->claveacceso = $_GET['clave'];
				$nivel->updatefactura();
